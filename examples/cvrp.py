...
def load_model(model_path):
    # 创建模型并加载权重
    test_model = AMPPO.load_from_checkpoint(model_path).to(device)
...
if __name__ == "__main__":
    ...
    if is_train:
        trainer.fit(model)
        trainer.save_checkpoint("./model/cvrp10-greedy.ckpt")

    # 取消下面的注释来加载和运行已保存的模型
    loaded_model = load_model("./model/cvrp10-greedy.ckpt")
