...
class HumanFeedbackCollector:
    ...
    def collect_feedback(self, model, num_pairs=10):
        ...
        for i in range(num_pairs):
            ...
            # 保存两个解的图片
            self.save_comparison_images(td_init[0], out1["actions"][0], out2["actions"][0], out1["reward"][0].item(), out2["reward"][0].item(), i)
    ...
    def save_comparison_images(self, td, action1, action2, reward1, reward2, pair_id):
        """保存对比图片"""
...
def main():
    ...
    # 创建环境
    env = HCVRPEnv(
        generator_params=dict(num_loc=60, num_agents=5),
    )
    
    model = PARCORLModule.load_from_checkpoint("../hcvrp_models/hcvrp_rl_model.ckpt").to(device)
    feedback_collector = HumanFeedbackCollector(env, save_dir="human_feedback")
    
    # 收集反馈
    print("\n开始收集人类反馈...")
    num_pairs = int(input("请输入要收集的反馈对数量 (默认10): ") or "10")
    feedback_data = feedback_collector.collect_feedback(model, num_pairs=num_pairs)
